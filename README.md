...
- **Go to Terminal Session** (⇧⌘T, also in the menu bar icon): brings the Teerminal, iTerm2 or Terminal session that is working in the document's folder to the front, so you can read what an agent wrote and jump straight back to it. Teerminal's persistent tmux sessions are found through `teerminalctl tmux list` (they outlive the app), Teerminal app sessions from its manifest, and iTerm2 / Terminal tabs by the working directory of the processes on their tty (Automation permission is asked once). A persistent session is focused in the Teerminal app when it is open, in the terminal tab already viewing it, or by attaching a new view in iTerm2 / Terminal. With no matching session, MD Reader starts one the way you configured in **Settings…**: by default a persistent `teerminalctl tmux run --cwd <folder> -- codex`, or a Teerminal preset, or a plain command in a new iTerm2 / Terminal window.
