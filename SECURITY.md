...
- **"Go to Terminal Session" talks to other apps.** It reads Teerminal's session manifest, runs `teerminalctl tmux list --json` (looked up in `~/.local/bin`, `/opt/homebrew/bin`, `/usr/local/bin`, never from PATH), sends
